...
namespace TheEWL
{
    public partial class TestListenWrite : UserControl
    {
        ...
        public TestListenWrite()
        {
            InitializeComponent();
        }
        private static TestListenWrite _instance;
        public static TestListenWrite Instance
        {
            get
            {
                if (_instance == null)
                    _instance = new TestListenWrite();
                return _instance;
            }
        ...
        void ButtonClickQ(object sender, EventArgs e)
        {
            ...
            {
                CurrID++;
                if ((string)btnPlay.Tag == txtAnswer.Text)
                {
                    Q(true);
                    MainTests.Instance.Say(1, 0);
                    MainTests.Instance.addWordList(SelectedWordMP3);
                }
                else
                {
                    Q(false);
        ...
        private void LoadQ()
        {
            if (CurrID < 10)
            {
                ...
                btnPlay.Tag = theListe[CurrID].getWord();
